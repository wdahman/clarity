    ...
    <div class="nav-content">
      ...
      <template v-for="(item, index) in items">
        <div class="nav-group" v-if="item.children">
          ...
          <div class="nav-group-children" v-bind:class="{ 'is-expanded': states[index] || activePage.path.startsWith(item.path) }" v-bind:style="{ height: (states[index]) ? `${item.children.length * 36}px` : '0'}">
            <router-link class="nav-link" :to="childItem.path" v-for="childItem in item.children" v-bind:class="{ active: childItem.path === activePage.path || childActive(childItem) }">
